...
def DownloadAllConception(): 
    ...
    while True != CHECKER.Contains(clsVal,"disabled"):  
        ...
        if ("Tables" in jsonData and []!=jsonData["Tables"]):
            ...
            postArr=[]
            for row in rows:
                conception=row[1].replace("*","@").strip()
                item={"Name":conception}
                postArr.append(item)

            serverUrl=Cfg.GetProcAPIUrl(keyName="ALLConception")
            post_data={"keyName":"ALLConception",  "jsonReq": json.dumps({},ensure_ascii=False),"jsonRes": json.dumps(postArr,ensure_ascii=False)}
