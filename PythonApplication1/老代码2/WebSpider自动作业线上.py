...
def StartBXCGTaskCreate():
    ...
    threadCount=psutil.cpu_count(True)
    path=r"%s\WebSpider北向持股明细任务生成V2增量.py"%root
    sub=subprocess.Popen("cmd.exe /C python %s"%path,creationflags=subprocess.CREATE_NEW_CONSOLE)
    print("%s %s"%("北向持股明细任务生成",sub))
    time.sleep(20)
    pass
...
def StartBXCJTaskCreate():
    ...
    threadCount=psutil.cpu_count(True)
    path=r"%s\WebSpider北向成交明细任务生成V2增量.py"%root     
    sub=subprocess.Popen("cmd.exe /C python %s"%path,creationflags=subprocess.CREATE_NEW_CONSOLE)
    print(sub)
    time.sleep(20)
    pass
...
def InitialJob():
    ...
    r.DictSave(jobNamespace,"融资融券任务"
               ,{"Code":"融资融券任务"
               ,"Name":"融资融券任务"
               ,"StartTime":datetime.date.today().strftime("%Y-%m-%d 22:10:00") #每天定时执行
               ,"Status":"Pending"})
    r.DictSave(jobNamespace,"财务主要指标任务"
               ,{"Code":"财务主要指标任务"
               ,"Name":"财务主要指标任务"
               ,"StartTime":datetime.date.today().strftime("%Y-%m-%d 23:10:00") #每天定时执行
               ,"Status":"Pending"})
...
def ProcJob(dictName,key,val,pageIndex,pageCount,pageSize,curIndex,total):
    ...
    if "资金流向任务" == code and val["StartTime"]<=currentTime and currentTime.weekday()<6:
        ...
        StartRZRQTask()
        r.DictSave(dictName,key ,val)
    elif "财务主要指标任务" == code and val["StartTime"]<=currentTime and "Pending" == val["Status"]:
        val["StartTime"]=(val["StartTime"]+datetime.timedelta(days=1)).strftime("%Y-%m-%d %H:%M:%S")
        val["Status"]="Running"
        r.DictSave(dictName,key ,val)
        print("正在启动%s任务"%code)
        ...
        print("处理 %s"%key)
        r.DictSave(dictName,key ,val)
    elif "北向成交明细任务" == code and val["StartTime"]<=currentTime and  "Pending" == val["Status"]:
        val["StartTime"]=(val["StartTime"]+datetime.timedelta(days=1)).strftime("%Y-%m-%d %H:%M:%S")
        val["Status"]="Running"
        r.DictSave(dictName,key ,val)
        StartBXCJTask()
    elif "北向持股明细任务" == code and val["StartTime"]<=currentTime  and "Pending" == val["Status"]:
        val["StartTime"]=(val["StartTime"]+datetime.timedelta(days=1)).strftime("%Y-%m-%d %H:%M:%S")
        val["Status"]="Running"
        r.DictSave(dictName,key ,val)
        StartBXCGTask()
